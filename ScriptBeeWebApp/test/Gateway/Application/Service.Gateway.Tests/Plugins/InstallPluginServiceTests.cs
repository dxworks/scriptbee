...
public class InstallPluginServiceTests
{
    ...
    [Fact]
    public async Task GivenUrl_WhenProjectDoesNotExist_ThenReturnsProjectDoesNotExistsError()
    {
        var projectId = ProjectId.FromValue("project-id");
        _getProject
            .GetById(projectId, Arg.Any<CancellationToken>())
    ...
    [Fact]
    public async Task GivenUrl_WhenInstallReturnsManifestNotFound_ThenReturnsManifestNotFoundError()
    {
        var projectId = ProjectId.FromValue("project-id");
        var projectDetails = ProjectDetailsFixture.BasicProjectDetails(projectId);
        ...
        _bundlePluginInstaller
            .Install(projectId, "https://example.com/plugin.zip", Arg.Any<CancellationToken>())
            .Returns(new PluginManifestNotFoundError());
    ...
    [Fact]
    public async Task GivenUrl_WhenInstallFails_ThenReturnsInstallationError()
    {
        var projectId = ProjectId.FromValue("project-id");
        var pluginId = new PluginId("failedPlugin", new Version("1.0.0"));
        var projectDetails = ProjectDetailsFixture.BasicProjectDetails(projectId);
    ...
    [Fact]
    public async Task GivenUrl_WhenInstallSucceeds_ThenReturnsUpdatedProjectDetailsAndInstallsToInstances()
    {
        var projectId = ProjectId.FromValue("project-id");
        var pluginId = new PluginId("urlPlugin", new Version("3.0.0"));
        var projectDetails = ProjectDetailsFixture.BasicProjectDetails(projectId);
        var updatedProjectDetails = projectDetails with
        {
            InstalledPlugins = [new PluginInstallationConfig(pluginId.Name, pluginId.Version)],
