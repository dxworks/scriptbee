...
public sealed class ScriptsService : IScriptsService
{
    ...
    public async Task<
        OneOf<IEnumerable<ScriptFileStructureNode>, ProjectMissing>
    > GetScriptsStructureAsync(string projectId, CancellationToken cancellationToken = default)
    {
        var documentExists = await _projectModelService.DocumentExists(
            projectId,
            cancellationToken
        );
        if (!documentExists)
        ...
        var structure = await GetScriptFileStructureAsync(
            projectId,
            fileTreeNode,
            0,
            cancellationToken
    ...
    public async Task<
        OneOf<ScriptDataResponse, ProjectMissing, ScriptMissing>
    > GetScriptByFilePathAsync(
        string filepath,
        string projectId,
    ...
    )
    {
        ...
        var scriptModel = await _scriptModelService.GetScriptModelByFilePathAsync(
            filepath,
            projectId,
            cancellationToken
        );
    ...
    public async Task<OneOf<ScriptDataResponse, ProjectMissing, ScriptMissing>> GetScriptByIdAsync(
        string scriptId,
        string projectId,
        CancellationToken cancellationToken = default
    )
    ...
    public async Task<OneOf<string, ProjectMissing, ScriptMissing>> GetScriptContentAsync(
        string scriptId,
        string projectId,
        CancellationToken cancellationToken = default
    )
    {
        ...
        var content = await _projectFileStructureManager.GetFileContentAsync(
            projectId,
    ...
    public async Task<
        OneOf<ScriptDataResponse, ProjectMissing, ScriptConflict, InvalidScriptType>
    > CreateScriptAsync(CreateScript createScript, CancellationToken cancellationToken = default)
    {
        ...
        var (extension, content) = await _projectStructureService.GetSampleCodeAsync(
            createScript.ScriptLanguage,
            cancellationToken
        );
        ...
        var node = _projectFileStructureManager.CreateSrcFile(
            createScript.ProjectId,
    ...
    )
    {
        ...
        scriptModel.Parameters = updateScript
    ...
    public async Task<OneOf<ScriptDataResponse, ProjectMissing, ScriptMissing>> DeleteScriptAsync(
        string scriptId,
        string projectId,
        CancellationToken cancellationToken = default
    )
    ...
    private async Task<IEnumerable<ScriptFileStructureNode>> GetScriptFileStructureAsync(
        string projectId,
    ...
    )
    {
        ...
        foreach (var treeNode in fileTreeNode.Children)
        {
            ...
            {
                var node = await AddScriptToFileStructure(
                    projectId,
                    treeNode,
                    level,
                    cancellationToken
            ...
            {
                scriptFileStructureNodes.Add(
                    new ScriptFileStructureNode
                    {
                        IsDirectory = true,
                        Children = await GetScriptFileStructureAsync(
                            projectId,
                            treeNode,
                            level + 1,
                            cancellationToken
                        ...
                        AbsolutePath = _projectFileStructureManager.GetAbsoluteFilePath(
                            projectId,
                            treeNode.FilePath
                        ),
                        Level = level,
    ...
    private async Task<ScriptFileStructureNode> AddScriptToFileStructure(
        string projectId,
    ...
    )
    {
        ...
        {
            absoluteFilePath = _projectFileStructureManager.GetAbsoluteFilePath(
                projectId,
                treeNode.FilePath
            );
        }
    ...
    {
        return _generateScriptService
            .GetSupportedLanguages()
            .Any(language => language.Name == scriptType);
    }
    ...
    private static ScriptParameter CreateScriptParameterModel(
        EndpointDefinitions.Arguments.ScriptParameter parameter
    )
    {
