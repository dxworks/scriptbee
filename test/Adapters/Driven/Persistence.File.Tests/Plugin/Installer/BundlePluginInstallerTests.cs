...
public class BundlePluginInstallerTests
{
    ...
    private readonly ILogger<BundlePluginInstaller> _logger =
        Substitute.For<ILogger<BundlePluginInstaller>>();

    private readonly BundlePluginInstaller _bundlePluginInstaller;

    public BundlePluginInstallerTests()
    {
        _bundlePluginInstaller = new BundlePluginInstaller(_pluginReader, _simplePluginInstaller,
            _pluginUninstaller, _pluginFetcher, _logger);
    }
    ...
    {
        _pluginReader.ReadPlugins(ConfigFolders.PathToPlugins)
            .Returns(pluginList.Plugins);
        _pluginFetcher.GetPluginUrl("pluginId", "1.0.0")
            .Returns("url");
        _simplePluginInstaller.Install("url", "pluginId", "1.0.0", Arg.Any<CancellationToken>())
            .Returns("plugin_folder");

        var pluginFolders =
            await _bundlePluginInstaller.Install("pluginId", "1.0.0",
        ...
        PluginList pluginList)
    {
        _pluginReader.ReadPlugins(ConfigFolders.PathToPlugins)
            .Returns(pluginList.Plugins);
        _pluginFetcher.When(x => x.GetPluginUrl(Arg.Any<string>(), Arg.Any<string>()))
        ...
        PluginList pluginList)
    {
        ...
        _simplePluginInstaller
            .When(x => x.Install(Arg.Any<string>(), Arg.Any<string>(), Arg.Any<string>(),
                Arg.Any<CancellationToken>()))
            .Throws(new Exception());
    ...
    public async Task GivenSimplePluginAlreadyInstalled_WhenInstall_ThenNoPluginIsUninstalled()
    {
        ...
        _pluginFetcher.GetPluginUrl("pluginName", "1.0.0")
            .Returns("url");

        var pluginFolders =
            await _bundlePluginInstaller.Install("pluginName", "1.0.0",
                Arg.Any<CancellationToken>());

        Assert.Empty(pluginFolders);
        await _simplePluginInstaller.Received(0).Install(Arg.Any<string>(), Arg.Any<string>(),
            Arg.Any<string>(),
            Arg.Any<CancellationToken>());
        _pluginUninstaller.Received(0).Uninstall(Arg.Any<string>());
    }
    ...
    [Fact]
    public async Task
        GivenSimplePluginVersionAndExistingNewVersion_WhenInstall_ThenPluginIsNotInstalled()
    {
        _pluginReader.ReadPlugins(ConfigFolders.PathToPlugins)
            .Returns(new List<Domain.Model.Plugin.Plugin>
            {
    ...
    [Fact]
    public async Task
        GivenSimplePluginAndExistingOldVersion_WhenInstall_ThenOldVersionsAreUninstalled()
    {
        _pluginReader.ReadPlugins(ConfigFolders.PathToPlugins).Returns(
        ...
        _pluginFetcher.GetPluginUrl("pluginName", "14.3.1")
            .Returns("url");
        _simplePluginInstaller.Install("url", "pluginName", "14.3.1", Arg.Any<CancellationToken>())
            .Returns("plugin_folder");

        var pluginFolders =
            await _bundlePluginInstaller.Install("pluginName", "14.3.1",
                Arg.Any<CancellationToken>());
    ...
    private class BundlePluginInstallerSimpleTestData : IEnumerable<object[]>
    {
        public IEnumerator<object[]> GetEnumerator()
        {
            ...
            yield return
            [
                new PluginList([new TestPlugin("pluginName", new Version(1, 0, 0))])
            ];
            yield return
            [
                new PluginList([
    ...
    [Fact]
    public async Task
        GivenBundlePluginThatHasNoPluginInFolder_WhenInstall_ThenOnlyFolderIsInstalled()
    {
        _pluginReader.ReadPlugins(ConfigFolders.PathToPlugins)
            .Returns(new List<Domain.Model.Plugin.Plugin>());
        _pluginFetcher.GetPluginUrl("bundle", "1.0.0")
            .Returns("url");
        _simplePluginInstaller
            .Install("url", "bundle", "1.0.0", Arg.Any<CancellationToken>())
            .Returns("bundle_folder");
        _pluginReader.ReadPlugin("bundle_folder")
            .Returns((Domain.Model.Plugin.Plugin?)null);

        var pluginFolders =
            await _bundlePluginInstaller.Install("bundle", "1.0.0", Arg.Any<CancellationToken>());
    ...
    public async Task GivenBundlePluginWithMultiplePlugins_WhenInstall_ThenPluginsAreInstalled()
    {
        ...
        SetupBundlePlugins("bundle", "1.0.0", 4,
    ...
    public async Task GivenBundleWithBundles_WhenInstall_ThenPluginsAreInstalled()
    {
        ...
        _pluginFetcher.GetPluginUrl("bundle", "1.0.0")
            .Returns("url");
        _simplePluginInstaller.Install("url", "bundle", "1.0.0", Arg.Any<CancellationToken>())
            .Returns("bundle_folder");
        _pluginFetcher.GetPluginUrl("pluginId1", "1.0.0")
            .Returns("url_plugin1");
        _simplePluginInstaller
            .Install("url_plugin1", "pluginId1", "1.0.0", Arg.Any<CancellationToken>())
            .Returns("plugin_folder1");
        _pluginFetcher.GetPluginUrl("pluginId2", "1.0.0")
            .Returns("url_plugin2");
        _simplePluginInstaller
    ...
    [Fact]
    public async Task
        GivenBundlePluginWithMultiplePlugins_WhenInstall_ThenOldPluginsAreUninstalled()
    {
        _pluginReader.ReadPlugins(ConfigFolders.PathToPlugins)
            .Returns(new List<Domain.Model.Plugin.Plugin>
            {
        ...
        SetupBundlePlugins("bundle", "4.0.0", 2,
            new TestBundlePlugin(PluginKind.Linker, "linker", "1.0.0"));
    ...
    public async Task GivenBundlePluginThatFails_WhenInstall_ThenDataIsRemoved()
    {
        ...
        _pluginFetcher.GetPluginUrl("bundle", "1.0.0")
            .Returns("url");
        _pluginFetcher.When(x => x.GetPluginUrl("pluginId", "1.0.0"))
            .Throws(new Exception());
        _simplePluginInstaller.Install("url", "bundle", "1.0.0", Arg.Any<CancellationToken>())
            .Returns("bundle_folder");
        _pluginReader.ReadPlugin("bundle_folder").Returns(CreateBundlePlugin("bundle",
            "1.0.0",
        ...
        await _simplePluginInstaller.Received(0).Install(Arg.Any<string>(), "pluginId", "1.0.0",
            Arg.Any<CancellationToken>());
    }
    ...
    public async Task GivenBundleWithMultiplePluginsWhereOneFails_WhenInstall_ThenDataIsRemoved()
    {
        ...
        _simplePluginInstaller
            .Install("url_plugin1", "pluginId1", "1.0.0", Arg.Any<CancellationToken>())
            .Returns("plugin_folder1");
        _pluginFetcher.When(x => x.GetPluginUrl("pluginId2", "2.0.0"))
            .Throws(new Exception());
        _pluginReader.ReadPlugin("bundle_folder")
            .Returns(CreateBundlePlugin("bundle", "1.0.0",
                new TestBundlePlugin(PluginKind.Plugin, "pluginId1", "1.0.0"),
        ...
        await _simplePluginInstaller.Received(0).Install(Arg.Any<string>(), "pluginId2", "2.0.0",
            Arg.Any<CancellationToken>());

        await _simplePluginInstaller.Received(Quantity.Within(1, 3)).Install(Arg.Any<string>(),
            Arg.Any<string>(), "1.0.0",
            Arg.Any<CancellationToken>());
        _pluginUninstaller.Received(Quantity.Within(1, 3)).ForceUninstall(Arg.Any<string>());
    }

    private void SetupBundlePlugins(string bundleName, string bundleVersion, int pluginCount,
        params TestBundlePlugin[] bundleExtensionPoints)
    {
        _pluginFetcher.GetPluginUrl(bundleName, bundleVersion)
            .Returns("url");
        _simplePluginInstaller
