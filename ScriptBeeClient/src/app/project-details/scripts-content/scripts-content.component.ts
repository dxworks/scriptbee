...
export class ScriptsContentComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.store.select(selectProjectDetails).pipe(filter(x => !!x)).subscribe({
      next: projectDetails => {
    ...
    this.store.select(selectScriptTreeLeafClick).pipe(filter(x => !!x)).subscribe({
      next: node => {
    ...
    this.store.select(selectLastRunOutput).subscribe({
      next: outputState => {
        ...
        this.outputFiles = outputState.results.filter(x => x.type == 'File').map(x => ({
          fileId: x.id,
          fileName: x.name,
        }));
